...
(Table~\ref{tab:models}). Claude Code runs were planned but not executed: the
...
(provider prefixes dropped; ``Agentic-*'', ``Instruction-*'' and ``Reasoning-*''
are RiVault aliases). ``correct'' = compiled, ran and passed the self-check;
``wall'' is the median agent wall time. $^\dagger$Geometric mean
...
Three groups are visible in Table~\ref{tab:models}. The instruction-tuned
small/medium RiVault aliases and Llama-4-Scout finish in seconds, change
essentially nothing, and therefore score $\approx 1\times$ while being
``correct'' almost always. Seed-OSS-36B and RiVault/Instruction-Small attempt
changes but break the build or the self-check in roughly half of the runs. The
reasoning and agentic models (Kimi-K3, GLM-5.2, GLM-5.3-Flash, DeepSeek-V4-Flash,
Qwen3.8-27B, the RiVault Reasoning/Agentic aliases) keep 145--167 of 174 runs
correct with median speedups in the tens to hundreds.
